...
- The **agent loop** (`vibe/core/agent_loop/_loop.py`) is async — this is the core of the program
...
- `asyncio.run()` is the entry point into async code — find it in `vibe/cli/programmatic.py` and `vibe/core/utils/concurrency.py`
...
In `vibe/cli/update_notifier/update.py`, find where `fetch_update()` is awaited — only one gateway is used there. Write a function that queries **both in parallel** using `asyncio.gather` and compares timing vs. sequential awaits.

For a production example of the same pattern, read `MCPRegistry._discover_all()` in `vibe/core/tools/mcp/registry.py` — it uses `asyncio.gather` to discover multiple MCP servers concurrently.
