...
- **Config example** (already in mistral-vibe `README.md` under `MCP Server Configuration`):
  ...
  ```
- **Good codebase pointer**: `MCPRegistry._discover_all()` in `vibe/core/tools/mcp/registry.py` — shows `asyncio.gather` used to discover multiple servers in parallel (also relevant to session 8)
